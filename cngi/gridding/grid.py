...
def grid(vis_dataset,grid_parms):
      ...
      assert grid_parms_copy['chan_mode'] == 'continuum' or  grid_parms_copy['chan_mode'] == 'cube','The chan_mode parameter in grid_parms can only be \'continuum\' or \'cube\'.'
      ...
      grid_dict = {}
      coords = {'chan':freq_coords, 'pol':np.arange(n_imag_pol),'u':np.arange(grid_parms_copy['imsize'][0]),'v':np.arange(grid_parms_copy['imsize'][1])}
...
def _graph_grid(vis_dataset,grid_parms):	
      ...
      iter_chunks_indx = itertools.product(np.arange(n_chunks_in_each_dim[0]),np.arange(n_chunks_in_each_dim[1]),np.arange(n_chunks_in_each_dim[2]),np.arange(n_chunks_in_each_dim[3]))
      
      assert grid_parms['chan_mode'] == 'continuum' or  grid_parms['chan_mode'] == 'cube','The chan_mode parameter in grid_parms can only be \'continuum\' or \'cube\'.'
      ...
      for c_time, c_baseline, c_chan, c_pol in iter_chunks_indx:
          sub_grid_and_sum_weights  = dask.delayed(_standard_grid_dask)(vis_dataset.DATA.data.partitions[c_time,c_baseline,c_chan,c_pol],
          ...
          if grid_parms['chan_mode'] == 'continuum':
              sub_grid_and_sum_weights = [da.from_delayed(sub_grid_and_sum_weights[0],(1,chunk_sizes[3][c_pol],grid_parms['imsize'][0],grid_parms['imsize'][1]),dtype=np.complex128),
					  da.from_delayed(sub_grid_and_sum_weights[1],(1,chunk_sizes[3][c_pol]),dtype=np.float64)]
              list_of_sub_grids_and_sum_weights[0].append(sub_grid_and_sum_weights)
          elif grid_parms['chan_mode'] == 'cube':
              sub_grid_and_sum_weights = [da.from_delayed(sub_grid_and_sum_weights[0],(chunk_sizes[2][c_chan],chunk_sizes[3][c_pol],grid_parms['imsize'][0],grid_parms['imsize'][1]),dtype=np.complex128),
					  da.from_delayed(sub_grid_and_sum_weights[1],(chunk_sizes[2][c_chan],chunk_sizes[3][c_pol]),dtype=np.float64)]
          ...
          while len(list_of_sub_grids_and_sum_weights[c_chan]) > 1:
              ...
              for i in range(0, len(list_of_sub_grids_and_sum_weights[c_chan]), 2):
                  if i < len(list_of_sub_grids_and_sum_weights[c_chan])-1:
                      lazy = [da.add(list_of_sub_grids_and_sum_weights[c_chan][i][0], list_of_sub_grids_and_sum_weights[c_chan][i + 1][0]), da.add(list_of_sub_grids_and_sum_weights[c_chan][i][1], list_of_sub_grids_and_sum_weights[c_chan][i + 1][1])] 
                  else:
                      lazy = [list_of_sub_grids_and_sum_weights[c_chan][i][0], list_of_sub_grids_and_sum_weights[c_chan][i][1]]
          ...
          while len(list_of_sub_grids_and_sum_weights) > 1:
              ...
              for i in range(0, len(list_of_sub_grids_and_sum_weights), 2):
                  if i < len(list_of_sub_grids_and_sum_weights)-1:
                      lazy = [[da.concatenate([list_of_sub_grids_and_sum_weights[i][0][0], list_of_sub_grids_and_sum_weights[i + 1][0][0]],axis=0), da.concatenate([list_of_sub_grids_and_sum_weights[i][0][1], list_of_sub_grids_and_sum_weights[i + 1][0][1]],axis=0)]]
                  else:
      ...
      list_of_sub_grids_and_sum_weights[0][0][0] = da.moveaxis(list_of_sub_grids_and_sum_weights[0][0][0], [-2,-1], [0,1])
...
def _standard_grid_dask(vis_data, uvw, weight, flag_row, flag, freq_chan, cgk_1D, grid_parms):
      ...
      _standard_grid_jit(grid, sum_weight, vis_data, uvw, freq_chan, chan_map, pol_map, weight, flag_row,flag, cgk_1D, n_uv, delta_lm, support, oversampling)
