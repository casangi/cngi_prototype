...
def calc_image_cell_size(vis_xds, global_xds):
    """
    ...
    global_xds : xarray.core.dataset.Dataset
    ...
    f_min = np.min(vis_xds.chan)
    f_max = np.max(vis_xds.chan)
    D_min = np.min(global_xds.ANT_DISH_DIAMETER)
    
    #Calculate cell size using 7 pixels per beam
    cell = rad_to_arc*np.array([c/(np.max(vis_xds.UVW[:,:,0])*f_max),c/(np.max(vis_xds.UVW[:,:,1])*f_max)])/7
    
    #If cell sizes are within 20% of eachother use the smaller cell size for both.
