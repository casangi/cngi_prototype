...
def standard_grid(
    ...
    mask = da.from_array(
        flag_row[..., None, None, None, None] | flag[..., None, None], chunks=chunks
    )
    weighted_data = da.from_array(
        grid_data[..., None, None] * weight[..., None, :, None, None], chunks=chunks
    )
    support_idxs = (
        (np.mgrid[0:support, 0:support] - support // 2)
        .reshape(2, -1)
        .T[None, None, None, None, ...]
    )

    uv_scale = da.from_array(
        (freq_chan[:, None, None, None] * (grid_parms["delta_lm"] * n_uv / c))[
            None, None, ...
        ],
        chunks=chunks,
    )
    ...
    uv_center_offset_indx = da.floor(
        (uv_center_indx - uv_pos) * oversampling + 0.5
    ).astype(int)
    uv_offset_indx = da.fabs(
        oversampling * support_idxs + uv_center_offset_indx
    ).astype(int)
    ...
    def set_grid(weighted_data_c, uv_offset_indx_c, uv_indx_c, mask_c, support, n_uv):
        mask_c = np.where(
            mask_c.repeat(support, 4).ravel() == False
        )  # turn in to a filter
        conv = cgk_1D[uv_offset_indx_c.clip(0, len(cgk_1D) - 1)].prod(
            axis=5, keepdims=True
        )
        gvals_1d = (conv * weighted_data_c).ravel()[mask_c]
        gcoords_1d = np.ravel_multi_index(
            uv_indx_c.reshape(-1, 2).T, n_uv, mode="clip"
        )[mask_c]
        # norm = np.sum(conv)

        grid_chunk1d = np.bincount(
            gcoords_1d, np.real(gvals_1d), minlength=np.prod(n_uv)
        )
        grid_chunk1d = grid_chunk1d + 1j * np.bincount(
            gcoords_1d, np.imag(gvals_1d), minlength=np.prod(n_uv)
        )
    ...
    da_grid = da.map_blocks(
        ...
        chunks=tuple(n_uv) + weighted_data.chunksize,
    )
