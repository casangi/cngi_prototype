...
def read_vis(infile, partition=None, chunks=None, consolidated=True, overwrite_encoded_chunks=True, **kwargs):
    ...
    if infile.lower.startswith('s3'):
        ...
        if s3.isdir(s3_url):
            ...
            object_names = [object_dict['name'].split('/')[-1] for object_dict in contents_map[1:]]
            if 'time' and 'baseline' and 'chan' and 'pol' in object_names:
                ...
                if partition != s3_url.split('/')[-1]:
                    ...
                    print("Input to partition keyword argument does not match provided S3 URI")
                    partition = s3_url.split('/')[-1]
            ...
            if ('global' in partition):
                ...
                olg_dirs = [odg['name'].split('/')[-1] for odg in contents_map_global[1:] if odg['StorageClass'] == 'DIRECTORY']
            ...
            if isinstance(partition, list):
                for part in partition:
                    ...
                    if s3.isdir(uri):
                        ...
                        xds_list += [(uri.replace('global/',''), xarray.open_zarr(INPUT, chunks=chunks, consolidated=consolidated,overwrite_encoded_chunks=overwrite_encoded_chunks))]
                    else:
                        print(f"Requested partition {part} not found in dataset")
            else:
                uri = '/'.join([s3_url,partition])
                INPUT = s3fs.S3Map(root=uri, s3=s3, check=False)
                xds = xarray.open_zarr(INPUT, chunks=chunks, consolidated=consolidated, overwrite_encoded_chunks=overwrite_encoded_chunks)
                xds_list.append(xds)
