...
def _create_baseline_mat(ant_id_list, auto_corr_flag):
    ...
    assert (
        baseline_indx == n_baseline
    ), "Baseline matrix has incorrect number of entries."
    return baseline_mat
...
def _time_baseline_info(ms_ddi, n_row):
    ...
    n_baseline_per_timestep = np.append(
        n_baseline_per_timestep, n_row - time_step_indx[-1]
    )  # Add last timestep number of baselines
...
def _freq_pol_info(ms_ddi, infile, ddi, col_names):
    ...
    tb_tool_meta.open(
        infile + "/SPECTRAL_WINDOW", nomodify=True, lockoptions={"option": "usernoread"}
    )
    ...
    total_bandwidth = tb_tool_meta.getcol(
        "TOTAL_BANDWIDTH", spectral_window_id[ddi], 1
    )[0]
    n_chan = tb_tool_meta.getcol("NUM_CHAN", spectral_window_id[ddi], 1)[0]
    tb_tool_meta.close()

    tb_tool_meta.open(
        infile + "/POLARIZATION", nomodify=True, lockoptions={"option": "usernoread"}
    )
...
def _decompose_row_4d(
    data, n_row, chunk_time_col_indx, chunk_col_baseline_indx, selected_col
):
...
####################################################################
def convert_ms_data_ndim(
    col_name,
    selected_col,
    chunksize,
    n_baseline,
    chunk_time_col_indx,
    chunk_col_baseline_indx,
    n_chan,
    n_pol,
):
    ...
    ###########################################################################################
        ...
        "SIGMA",
        "WEIGHT",
    ]:  # n_row x n_pol -> n_time x n_baseline x n_pol
    ...
    ###########################################################################################
        ...
        "MODEL_DATA",
        "CORRECTED_DATA",
    ]:  # n_row x n_chan x n_pol -> n_time x n_baseline x n_chan x n_pol
        assert (dim[1] == n_chan) & (dim[2] == n_pol), "Collumn dimentions not correct"

        # Flag all missing values
        if col_name == "FLAG":
            data = np.ones(
                (chunksize, n_baseline, dim[1], dim[2]), dtype=selected_col.dtype
            )
        else:
            data = np.zeros(
                (chunksize, n_baseline, dim[1], dim[2]), dtype=selected_col.dtype
...
def _processDDI(ddi, infile, outfile, compressor):
    ...
    (
        ...
        ant1_col,
        ant2_col,
    ) = _time_baseline_info(ms_ddi, n_row)
    # print('time_baseline_info time ', time.time() - start)

    # start = time.time()
    n_chan, n_pol, ref_frequency, chan_freq = _freq_pol_info(
        ms_ddi, infile, ddi, col_names
    )
    ...
    for cc, start_row_indx in enumerate(range(0, n_time, chunksize)):
        ...
        n_row_in_chunk = np.sum(
            n_baseline_per_timestep[start_row_indx : start_row_indx + current_chunksize]
        )
        ant1_chunk = ant1_col[
            time_step_indx[start_row_indx] : time_step_indx[start_row_indx]
            + n_row_in_chunk
        ]
        ant2_chunk = ant2_col[
            time_step_indx[start_row_indx] : time_step_indx[start_row_indx]
        ...
        for col_name in col_names:
            # try:
            if True:
                if col_name in ["FLAG_CATEGORY", "DATA_DESC_ID"]:
                    continue

                selected_col = ms_ddi.getcol(
                    col_name, time_step_indx[start_row_indx], n_row_in_chunk
                ).transpose()
                if selected_col.dtype == "bool":
                    selected_col = selected_col.astype(np.int)

                # start = time.time()
                data, dim_names, skip = convert_ms_data_ndim(
                    col_name,
                    selected_col,
                    current_chunksize,
                    n_baseline,
    ...
    coords["scan_number"] = (
        "time",
        da.from_array(scan_number_step, chunks=(chunksize,)),
    )
    ...
    no_repeat_scan_number, scan_number_step_indx = _no_repeat_array(
        scan_number_col.astype(int)
    )
    scan_number_indx = np.array(
        [list(no_repeat_scan_number), list(scan_number_step_indx)], np.int
    )

    field_id_col = ms_ddi.getcol("FIELD_ID").transpose()
    no_repeat_field_id, field_id_step_indx = _no_repeat_array(
        field_id_col.astype(int)
    )  # (2 x [n_field x 1])
    field_id_indx = np.array(
        [list(no_repeat_field_id), list(field_id_step_indx)], np.int
    )
...
def ms_to_zarr_numba(infile, outfile=None, ddi=None, compressor=None):
    ...
    ddis = tb_tool.taql("select distinct DATA_DESC_ID from %s" % prefix + ".ms").getcol(
        "DATA_DESC_ID"
    )
    tb_tool.close()
