...
def ms_to_zarr(infile, outfile=None, ddi=None, compressor=None):
    ...
    if os.path.isdir(os.path.join(infile, tables[-1])):
        ms_meta.open(os.path.join(infile, tables[-1]), nomodify=True, lockoptions={'option': 'usernoread'})
        if ms_meta.nrows() > 0:
            mcoords['spw'] = ms_meta.taql('select distinct SPECTRAL_WINDOW_ID from %s' % os.path.join(infile, tables[-1])).getcol('SPECTRAL_WINDOW_ID')
            mcoords['feed'] = ms_meta.taql('select distinct FEED_ID from %s' % os.path.join(infile, tables[-1])).getcol('FEED_ID')
            mcoords['receptors'] = list(range(np.max(ms_meta.taql('select distinct NUM_RECEPTORS from %s' % os.path.join(infile, tables[-1])).getcol('NUM_RECEPTORS'))))
            ...
            for col in ms_meta.colnames():
                ...
                if ms_meta.isvarcol(col):
                    ...
                    if col == 'BEAM_OFFSET': tshape, tdim = (2,len(mcoords['receptors'])), ('d2', 'receptors')
                    elif col == 'POL_RESPONSE': tshape, tdim = (len(mcoords['receptors']),len(mcoords['receptors'])), ('receptors', 'receptors')
    ...
    if os.path.isdir(os.path.join(infile, tables[-1])):
        ...
        if ms_meta.nrows() > 0:
            ...
            tshape = (2, max_poly)
            if ms_meta.nrows() != (len(mcoords['point_time'])*len(mcoords['antenna'])): print('WARNING: index mismatch in %s table' % tables[-1])
            for col in ms_meta.colnames():
    ...
    def processDDI(ddi, infile, outfile, compressor):
        ...
        for cc, start_row_indx in enumerate(range(0, n_time, chunksize)):
            rtestimate = ', remaining time est %s s' % str(int(((time.time()-start_ddi)/cc)*(n_time/chunksize-cc))) if cc > 0 else ''
            ...
            x_dataset = xarray.Dataset(chunkdata, coords=coords).chunk({'time': chunksize, 'baseline': None, 'chan': None, 'pol': None, 'uvw': None})
