...
def ms_to_zarr(
    infile, outfile=None, ddi=None, compressor=None, chunk_shape=(100, 400, 20, 1)
):
    """
    ...
    ddis = MS.taql("select distinct DATA_DESC_ID from %s" % prefix + ".ms").getcol(
        "DATA_DESC_ID"
    )
    ...
    def apad(arr, ts):
        return (
            np.pad(arr, (0, ts[0] - arr.shape[0]))
            if arr.ndim == 1
            else np.pad(arr, ((0, ts[0] - arr.shape[0]), (0, ts[1] - arr.shape[1])))
        )
    ...
    if os.path.isdir(os.path.join(infile, tables[-1])):
        ...
        if ms_meta.nrows() > 0:
            ...
            antidx, spwidx, feedidx = (
                ms_meta.getcol("ANTENNA_ID"),
                ms_meta.getcol("SPECTRAL_WINDOW_ID"),
                ms_meta.getcol("FEED_ID"),
            )
            if ms_meta.nrows() != (
                len(np.unique(antidx))
                * len(np.unique(spwidx))
                * len(np.unique(feedidx))
            ):
            ...
            for col in ms_meta.colnames():
                ...
                if ms_meta.isvarcol(col):
                    ...
                    if col == "BEAM_OFFSET":
                        tshape, tdim = (
                            (2, len(mcoords["receptors"])),
                            ("d2", "receptors"),
                        )
                    elif col == "POL_RESPONSE":
                        tshape, tdim = (
                            (len(mcoords["receptors"]), len(mcoords["receptors"])),
                            ("receptors", "receptors"),
                        )
                    ...
                    if data.ndim == 1:
                        ...
                        mvars["FEED_" + col] = xarray.DataArray(
                            metadata,
                            dims=["spw", "antenna", "feed", "d" + str(data.shape[1])],
                        )
        ms_meta.close()
    ...
    if os.path.isdir(os.path.join(infile, tables[-1])):
        ...
        if ms_meta.nrows() > 0:
            funique, fidx, fcount = np.unique(
                ms_meta.getcol("NAME"), return_inverse=True, return_counts=True
            )
            mcoords["field"] = [
                funique[ii] if fcount[ii] == 1 else funique[ii] + " (%s)" % str(nn)
                for nn, ii in enumerate(fidx)
            ]
            max_poly = (
                np.max(
                    ms_meta.taql(
                        "select distinct NUM_POLY from %s"
                        % os.path.join(infile, tables[-1])
                    ).getcol("NUM_POLY")
                )
                + 1
    ...
    if os.path.isdir(os.path.join(infile, tables[-1])):
        ...
        if ms_meta.nrows() > 0:
            mcoords["time_fcmd"], timeidx = np.unique(
                convert_time(ms_meta.getcol("TIME")), return_inverse=True
            )
            for col in ms_meta.colnames():
                if not ms_meta.iscelldefined(col, 0):
                    continue
                ...
                data = ms_meta.getcol(col).transpose()
                metadata = np.full(
                    (len(mcoords["time_fcmd"])), np.nan, dtype=data.dtype
                )
    ...
    if os.path.isdir(os.path.join(infile, tables[-1])):
        ...
        if ms_meta.nrows() > 0:
            mcoords["time_hist"], timeidx = np.unique(
                convert_time(ms_meta.getcol("TIME")), return_inverse=True
            )
            for col in ms_meta.colnames():
                if not ms_meta.iscelldefined(col, 0):
                ...
                data = ms_meta.getcol(col).transpose()
                metadata = np.full(
                    (len(mcoords["time_hist"])), np.nan, dtype=data.dtype
                )
                metadata[timeidx] = data
    ...
    if os.path.isdir(os.path.join(infile, tables[-1])):
        ...
        if ms_meta.nrows() > 0:
            funique, fidx, fcount = np.unique(
                ms_meta.getcol("PROJECT"), return_inverse=True, return_counts=True
            )
            mcoords["observation"] = [
                funique[ii] if fcount[ii] == 1 else funique[ii] + " (%s)" % str(nn)
                for nn, ii in enumerate(fidx)
    ...
    if os.path.isdir(os.path.join(infile, tables[-1])):
        ...
        if ms_meta.nrows() > 0:
            mcoords["time_point"], timeidx = np.unique(
                convert_time(ms_meta.getcol("TIME")), return_inverse=True
            )
            ...
            for col in ms_meta.colnames():
                ...
                try:  # can't use getvarcol as it dies on large tables like this
                    ...
                    if data.ndim == 1:
                        metadata = np.full(
                            (len(mcoords["time_point"]), len(mcoords["antenna"])),
                            np.nan,
                            dtype=data.dtype,
                        )
    ...
    if os.path.isdir(os.path.join(infile, tables[-1])):
        ...
        if ms_meta.nrows() > 0:
            funique, fidx, fcount = np.unique(
                ms_meta.getcol("TYPE"), return_inverse=True, return_counts=True
            )
            mcoords["processor"] = [
                funique[ii] if fcount[ii] == 1 else funique[ii] + " (%s)" % str(nn)
                for nn, ii in enumerate(fidx)
    ...
    if os.path.isdir(os.path.join(infile, tables[-1])):
        ...
        if ms_meta.nrows() > 0:
            ...
            srcidx, spwidx = (
                ms_meta.getcol("SOURCE_ID"),
                ms_meta.getcol("SPECTRAL_WINDOW_ID"),
            )
            tshape = (2, max_lines)
            for col in ms_meta.colnames():
                try:
                    if col in ["SOURCE_ID", "SPECTRAL_WINDOW_ID"]:
                        continue
                    if not ms_meta.iscelldefined(col, 0):
                        continue
                    if (
                        ms_meta.isvarcol(col)
                        and (tshape[1] > 0)
                        and (col not in ["POSITION", "SOURCE_MODEL", "PULSAR_ID"])
                    ):
                        data = ms_meta.getvarcol(col)
                        data = np.array(
                            [
                        ...
                        metadata = np.full(
                            (len(mcoords["spw"]), len(mcoords["source"])) + tshape,
                            np.nan,
                            dtype=data.dtype,
                        )
                        metadata[spwidx, srcidx] = data
                        mvars["SRC_" + col] = xarray.DataArray(
                            metadata, dims=["spw", "source", "d" + str(max_lines)]
                        )
                    else:
                        data = ms_meta.getcol(col).transpose()
                        if col == "TIME":
                        ...
                        if data.ndim == 1:
                            ...
                            mvars["SRC_" + col] = xarray.DataArray(
                                metadata,
                                dims=["spw", "source", "d" + str(data.shape[1])],
                            )
                except Exception:
                    print(
                        "WARNING : unable to process col %s of table %s"
    ...
    if os.path.isdir(os.path.join(infile, tables[-1])):
        ...
        if ms_meta.nrows() > 0:
            funique, fidx, fcount = np.unique(
                ms_meta.getcol("OBS_MODE"), return_inverse=True, return_counts=True
            )
            mcoords["state"] = [
                funique[ii] if fcount[ii] == 1 else funique[ii] + " (%s)" % str(nn)
                for nn, ii in enumerate(fidx)
    ...
    for ii, tt in enumerate(other_tables.keys()):
        print(
            "processing support table %s of %s : %s"
            % (str(ii), str(len(other_tables.keys())), tt),
            end="\r",
        )
        ms_meta.open(
            os.path.join(infile, tt),
            nomodify=True,
            lockoptions={"option": "usernoread"},
        )
        if ms_meta.nrows() == 0:
        ...
        for col in ms_meta.colnames():
            ...
            if ms_meta.isvarcol(col):
                ...
                data = [
                    ...
                    if not isinstance(data["r" + str(kk)], bool)
                    else []
                    for kk in np.arange(len(data)) + 1
                ]
    ...
    def processDDI(ddi, infile, outfile, compressor, chunk_shape):
        ...
        tb_tool = tb()
        tb_tool.open(
            infile, nomodify=True, lockoptions={"option": "usernoread"}
        ...
        unique_times, time_changes, time_idxs = np.unique(
            times, return_index=True, return_inverse=True
        )
        ...
        unique_baselines, baseline_idxs = np.unique(
            ant1_ant2, axis=0, return_inverse=True
        )
        ...
        tb_tool_meta.open(
            os.path.join(infile, "SPECTRAL_WINDOW"),
            nomodify=True,
            lockoptions={"option": "usernoread"},
        )
        for col in tb_tool_meta.colnames():
            try:
                if not tb_tool_meta.iscelldefined(col, spw_id):
                    continue
                if col in ["FLAG_ROW"]:
                    continue
                if col in ["CHAN_FREQ", "CHAN_WIDTH", "EFFECTIVE_BW", "RESOLUTION"]:
                    aux_coords[col.lower()] = (
                        "chan",
                        tb_tool_meta.getcol(col, spw_id, 1)[:, 0],
                    )
                else:
                    meta_attrs[col] = tb_tool_meta.getcol(col, spw_id, 1).transpose()[0]
            except Exception:
                print(
                    "WARNING : unable to process col %s of table %s" % (col, tables[-1])
                )
        tb_tool_meta.close()

        tb_tool_meta.open(
            os.path.join(infile, "POLARIZATION"),
            nomodify=True,
            lockoptions={"option": "usernoread"},
        )
        for col in tb_tool_meta.colnames():
            if col == "CORR_TYPE":
                aux_coords[col.lower()] = (
                    "pol",
                    tb_tool_meta.getcol(col, pol_id, 1)[:, 0],
                )
        ...
        for cc, start_row_indx in enumerate(range(0, n_time, chunk_shape[0])):
            rtestimate = (
                ", remaining time est %s s"
                % str(
                    int(
                        ((time.time() - start_ddi) / cc)
                        * (n_time / chunk_shape[0] - cc)
                    )
                )
                if cc > 0
                else ""
            ...
            print(
                "processing chunk %s of %s" % (str(cc), str(n_time // chunk_shape[0]))
                + rtestimate,
                end="\r",
            )
            chunk = (
                np.arange(min(chunk_shape[0], n_time - start_row_indx)) + start_row_indx
            )
            chunk_time_changes = (
                time_changes[chunk] - time_changes[chunk[0]]
            )  # indices in this chunk of data where time value changes
            end_idx = (
                time_changes[chunk[-1] + 1]
                if chunk[-1] + 1 < len(time_changes)
                else len(time_idxs)
            )
            idx_range = np.arange(
                time_changes[chunk[0]], end_idx
            )  # indices (rows) in main table to be read
            coords.update({"time": unique_times[chunk]})

            chunkdata = {}
            for col in ms_ddi.colnames():
                if col in ["DATA_DESC_ID", "TIME"]:
                    continue
                if not ms_ddi.iscelldefined(col, idx_range[0]):
                    continue
                ...
                if col in "UVW":  # n_row x 3 -> n_time x n_baseline x 3
                    fulldata = np.full(
                        (len(chunk), n_baseline, data.shape[1]),
                        np.nan,
                        dtype=data.dtype,
                    )
                    fulldata[
                        time_idxs[idx_range] - chunk[0], baseline_idxs[idx_range], :
                    ] = data
                    ...
                    if col == "FIELD_ID" and "field" in mxds.coords:
                        coords["field"] = (
                            "time",
                            mxds.coords["field"].values[data[chunk_time_changes]],
                        )
                        ...
                        coords["processor"] = (
                            "time",
                            mxds.coords["processor"].values[data[chunk_time_changes]],
                        )
                    elif col == "OBSERVATION_ID" and "observation" in mxds.coords:
                        coords["observation"] = (
                            "time",
                            mxds.coords["observation"].values[data[chunk_time_changes]],
                        )
                    elif col == "STATE_ID" and "state" in mxds.coords:
                        coords["state"] = (
                            "time",
                            mxds.coords["state"].values[data[chunk_time_changes]],
                        )
                    else:
                        fulldata = np.full(
                            (len(chunk), n_baseline), np.nan, dtype=data.dtype
                        )
                        if col == "FLAG_ROW":
                            fulldata = np.ones(
                                (len(chunk), n_baseline), dtype=data.dtype
                            )
                        fulldata[
                            time_idxs[idx_range] - chunk[0], baseline_idxs[idx_range]
                        ] = data
                    ...
                    fulldata = np.full(
                        (len(chunk), n_baseline, n_pol), np.nan, dtype=data.dtype
                    )
                    fulldata[
                        time_idxs[idx_range] - chunk[0], baseline_idxs[idx_range], :
                    ] = data
                    ...
                    fulldata = np.full(
                        (len(chunk), n_baseline, n_chan), np.nan, dtype=data.dtype
                    )
                    fulldata[
                        time_idxs[idx_range] - chunk[0], baseline_idxs[idx_range], :
                    ] = data
                    ...
                    assert (data.shape[1] == n_chan) & (
                        data.shape[2] == n_pol
                    ), "Column dimensions not correct"
                    if col == "FLAG":
                        fulldata = np.ones(
                            (len(chunk), n_baseline, n_chan, n_pol), dtype=data.dtype
                        )
                    else:
                        fulldata = np.full(
            ...
            if cc == 0:
                encoding = dict(
                    zip(list(x_dataset.data_vars), cycle([{"compressor": compressor}]))
                )
                x_dataset.to_zarr(
                    outfile + "/" + str(ddi),
                    mode="w",
