...
def image_to_zarr(infile, outfile=None, artifacts=None):
    ...
    if artifacts == None:
        ...
        if suffix not in imtypes:
            imtypes = [suffix] + imtypes
    else:
    ...
    for imtype in imtypes:
        if os.path.exists(prefix + "." + imtype):
            ...
            summary = IA.summary(
                list=False
            )  # imhead would be better but chokes on big images
            ims = tuple(IA.shape())  # image shape
            coord_names = [
                ss.replace(" ", "_").lower().replace("stokes", "pol")
                for ss in summary["axisnames"]
            ]
            ...
            sphr_dims = [
                dd for dd in range(len(ims)) if summary["axisunits"][dd] == "rad"
            ]
            coord_idxs = np.mgrid[
                [
            ...
            coords = dict(
                [
                    (coord_names[dd], (spi, coord_world[di]))
            ...
            for dd, cs in enumerate(list(coord_world)):
                spi = tuple(
                    [slice(None) if di == dd else slice(1) for di in range(cs.ndim)]
                )
            ...
            tm["attrs"].update(
                dict(
                    [
                        (kk, list(nested_to_record(summary[kk], sep=".").items()))
                        for kk in nested
                    ]
                )
            )
    ...
    for chan in range(dsize[chan_dim]):
        print(
            "processing channel " + str(chan + 1) + " of " + str(dsize[chan_dim]),
            end="\r",
        )
        pt[chan_dim] = chan
        ...
        for imtype in imtypes:
            ...
            if imtype == "mask":
                ...
                xdas[imtype] = xa(
                    imchunk.reshape(imchunk.shape[2], 1), dims=["pol", "frequency"]
                )
            else:
        ...
        if chan == 0:
            xds = xd(
                xdas,
                coords=chunk_coords,
                attrs=nested_to_record(meta["attrs"], sep="_"),
            )
            encoding = dict(
                zip(list(xds.data_vars), cycle([{"compressor": compressor}]))
            )
    ...
    print(
        ...
        + str(dsize)
        + " in "
        + str(np.float32(time.time() - begin))
        + " seconds"
    )
