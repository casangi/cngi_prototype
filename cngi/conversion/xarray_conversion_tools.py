...
@jit(nopython=True)
def no_repeat_array(data):
      """
      Takes in an scalar array such as np.array([1,2,3,3,3,4,4,5,6,2,1,1])
      and returns a non repeating array np.array([1,2,3,4,5,6,2,1]) and index array
      where the transistion indices are given np.array([0,1,2,5,7,8,9,10]).
      Parameters
      ----------
      data : array
        A one dimentional scalar array

      Returns
      -------
      no_repeat_data : array
        Non repeating array
      step_indx_with_0: array
        Index array
      """    
...
def create_baseline_mat(ant_id_list,auto_corr_flag):
...
def time_baseline_info(ms_ddi,n_row):
    ...
    n_baseline_per_timestep = np.append(n_baseline_per_timestep,n_row-time_step_indx[-1]) #Add last timestep number of baselines
    ...
    n_ant = len(ant_id_list)
    baseline_mat = create_baseline_mat(ant_id_list,auto_corr_flag)
...
def decompose_row_4d(data,n_row, chunk_time_col_indx, chunk_col_baseline_indx, selected_col):
...
@jit(nopython=True,cache=True)
def decompose_row_3d(data,n_row, chunk_time_col_indx, chunk_col_baseline_indx, selected_col):
    for row in range(n_row):
      time_indx = chunk_time_col_indx[row]
      baseline_indx = chunk_col_baseline_indx[row]
      data[time_indx, baseline_indx,:] = selected_col[row,:]
...
@jit(nopython=True,cache=True)
def decompose_row_2d(data,n_row, chunk_time_col_indx, chunk_col_baseline_indx, selected_col):
    for row in range(n_row):
      time_indx = chunk_time_col_indx[row]
      baseline_indx = chunk_col_baseline_indx[row]
      data[time_indx, baseline_indx] = selected_col[row]
...
####################################################################
def convert_ms_data_ndim(col_name,selected_col,chunksize,n_baseline,chunk_time_col_indx,chunk_col_baseline_indx,n_chan,n_pol):
...
def processDDI(ddi, infile, outfile, compressor):
    ...
    start = time.time()
    ms_ddi = tb_tool.taql('select * from %s where DATA_DESC_ID = %s ORDERBY FIELD_ID,SCAN_NUMBER,TIME,ANTENNA1,ANTENNA2' % (infile,str(ddi)))
    ...
    #start = time.time()
    n_time, n_ant, n_baseline, time_step, time_step_indx, n_baseline_per_timestep, auto_corr_flag, baseline_mat, baseline_ant_pairs, ant1_col, ant2_col = time_baseline_info(ms_ddi,n_row)
    #print('time_baseline_info time ', time.time() - start)
    
    #start = time.time()
    n_chan, n_pol, ref_frequency, chan_freq = freq_pol_info(ms_ddi,infile,ddi,col_names)
    ...
    for cc,start_row_indx in enumerate(range(0,n_time,chunksize)):
      ...
      coords, dict_x_data_array = {'time':time_chunk,'baseline':np.arange(n_baseline),'chan':chan_freq,'pol':np.arange(n_pol),'uvw':np.arange(3)}, {}
      ...
      for col_name in col_names:
        try:
          ...
          #start = time.time()
          data, dim_names, skip = convert_ms_data_ndim(col_name,selected_col,current_chunksize,n_baseline,chunk_time_col_indx,chunk_col_baseline_indx,n_chan,n_pol)
    ...
    coords['interval'] =  ('time', interval_step)
