    ...
    for dv in xds.data_vars:
        ...
        if dv in vwcds:
            if (dv == "DATA") and ("SIGMA_SPECTRUM" in vwcds):
                xda = (
                    (xds.DATA / xds.SIGMA_SPECTRUM ** 2)
                    .coarsen(chan=width, boundary="trim")
                    .sum()
                )
                xda = (
                    xda
                    * (xds.SIGMA_SPECTRUM ** 2)
                    .coarsen(chan=width, boundary="trim")
                    .sum()
                )
            elif (dv == "CORRECTED_DATA") and ("WEIGHT_SPECTRUM" in vwcds):
                xda = (
                    (xds.CORRECTED_DATA * xds.WEIGHT_SPECTRUM)
                    .coarsen(chan=width, boundary="trim")
                    .sum()
                )
                xda = (
                    xda / xds.WEIGHT_SPECTRUM.coarsen(chan=width, boundary="trim").sum()
                )
            else:
                # .mean() produces runtimewarning errors (still works though), using .sum() / width is cleaner
                xda = (xda.coarsen(chan=width, boundary="trim").sum() / width).astype(
                    xds.data_vars[dv].dtype
                )
