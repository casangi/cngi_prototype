...
def timeaverage(xds, width=1, timespan="state", timebin=None):
    ...
    if timespan == "none":
        scans = [
            xds.where(xds.scan == ss, drop=True) for ss in np.unique(xds.scan.values)
        ]
        for scan in scans:
            ssds += [
                scan.where(scan.state == ss, drop=True)
                for ss in np.unique(scan.state.values)
            ]
    elif timespan == "scan":  # span across scans by separating out states
        ssds = [
            xds.where(xds.state == ss, drop=True) for ss in np.unique(xds.state.values)
        ]
        ...
        for dv in ss.data_vars:
            ...
            if dv in vwtds:
                if (dv == "DATA") and ("SIGMA_SPECTRUM" in ss.data_vars):
                    ...
                    xda = (
                        (ss.DATA * weight_spectrum)
                        .rolling(time=width, min_periods=1, center=True)
                        .sum()
                        / weight_spectrum.rolling(
                            time=width, min_periods=1, center=True
                        ).sum()
                    )
                elif (dv == "CORRECTED_DATA") and ("WEIGHT_SPECTRUM" in ss.data_vars):
                    xda = (
                        (ss.CORRECTED_DATA * ss.WEIGHT_SPECTRUM)
                        .rolling(time=width, min_periods=1, center=True)
                        .sum()
                        / ss.WEIGHT_SPECTRUM.rolling(
                            time=width, min_periods=1, center=True
                        ).sum()
                    )
