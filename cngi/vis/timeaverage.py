...
def timeaverage(xds, width=1, timespan='state', timebin=None):
    ...
    def group_average(gxds):
        ...
        if ('DATA' in gxds.data_vars) and ('WEIGHT' in gxds.data_vars):
            xda = (gxds.DATA / gxds.WEIGHT**0.5).coarsen(time=width, boundary='pad').sum()
            nxds['DATA'] = xda * (gxds.WEIGHT**0.5).coarsen(time=width, boundary='pad').sum()
        ...
        if ('CORRECTED_DATA' in gxds.data_vars) and ('WEIGHT' in gxds.data_vars):
    ...
    if (timespan == 'none') or (timespan is None):
        cgps = [group_average(sgp[1].unstack('stacked_time_baseline')) for gp in nxds.groupby('SCAN_NUMBER') for sgp in gp[1].groupby('STATE_ID')]
    elif timespan == 'scan':  # span across scans by separating out states
        cgps = [group_average(gp[1].unstack('stacked_time_baseline')) for gp in nxds.groupby('STATE_ID')]
    elif timespan == 'state':  # span across state by separating out scans
        cgps = [group_average(gp[1].unstack('stacked_time_baseline')) for gp in nxds.groupby('SCAN_NUMBER')]
