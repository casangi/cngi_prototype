...
def test_serial_gridder(show_plots=False):
    ...
    n_xy = np.array([250,
                     250])  # 2048                                                                 # 2 element array containing number of pixels for x (u and l) and y (v and m) axes.
    ...
    vis_grid, sum_weight = serial_grid(grid_data, uvw, weight, flag_row, flag, freq_chan, chan_map, n_chan, pol_map, cgk_1D, grid_parms)
    ...
    rms_error_corrected_dirty_image = np.linalg.norm(corrected_dirty_image - ximage_dataset.data_vars['corrected_dirty_image'].values,
                                                     'fro')
    ...
    if (max_error_grid < 3.552e-09) and (max_error_corrected_dirty_image < 9.795e-08) and (rms_error_grid < 1.601e-08) and (
            rms_error_corrected_dirty_image < 2.091e-06):
        print('Test Pass')
        pass_test = True
    else:
        print('Test Fail')
