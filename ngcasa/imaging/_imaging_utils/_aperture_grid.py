...
def _aperture_weight_grid_jit(grid, sum_weight, uvw, freq_chan, chan_map, pol_map, cf_baseline_map, cf_chan_map, cf_pol_map, imaging_weight, weight_conv_kernel, n_uv, delta_lm, weight_support, oversampling, field, phase_gradient):
    ...
    for i_time in range(n_time):
        ...
        #weight_conv_kernel_phase_gradient = weight_conv_kernel
                        ...
                        for i_pol in range(n_pol):
                            ...
                            if ~np.isnan(weighted_data) and (weighted_data != 0.0):
                                ...
                                for i_v in range(start_support[1],end_support[1]):
                                    ...
                                    cf_v_indx = oversampling[1]*i_v + v_center_offset_indx + conv_v_center

                                    for i_u in range(start_support[0],end_support[0]):
                                        u_indx = u_center_indx + i_u
                                        cf_u_indx = oversampling[0]*i_u + u_center_offset_indx + conv_u_center
...
'''
def _graph_aperture_weight_grid(vis_dataset,cf_dataset,grid_parms):
    import dask
    import dask.array as da
    import xarray as xr
    import time
    ...
    for c_time, c_baseline, c_chan, c_pol in iter_chunks_indx:
        ...
        cf_dataset["WEIGHT_CONV_KERNEL"].data,
        cf_dataset["SUPPORT"].data,
        cf_dataset["PHASE_GRADIENT"].data,
        freq_chan.partitions[c_chan],
        dask.delayed(grid_parms))
        grid_dtype = np.complex128
...
#When jit is used round is replaced by standard c++ round that is different to python round
@jit(nopython=True, cache=True, nogil=True)
