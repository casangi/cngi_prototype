...
def test_serial_gridder(show_plots=False):
    ...
    weight[:, :, 0] = (
        weight[:, :, 0] + weight[:, :, 1]
    ) / 2  # Casa averages the weights for different polarizations. See code/msvis/MSVis/VisImagingWeight.cc VisImagingWeight::unPolChanWeight
    ...
    # Grid data
    vis_grid, sum_weight = serial_grid(
        grid_data,
        uvw,
        weight,
        flag_row,
