...
def test_dask_serial_gridder(show_plots=False):
    ...
    n_xy = np.array(
        [200, 400]
    )  # 2048                                                                 # 2 element array containing number of pixels for x (u and l) and y (v and m) axes.
    padding = 1.2  # Padding factor
    n_xy_padded = (padding * n_xy).astype(
        int
    )  # 2 element array containg number of pixels with padding
    delta_lm = np.array(
        [-0.08 * dtr, 0.08 * dtr]
    ...
    delta_uv = np.array(
        [1 / (n_xy_padded[0] * delta_lm[0]), 1 / (n_xy_padded[1] * delta_lm[1])]
    )  # Visibility domain cell size
    ...
    cgk, cgk_image = gck.create_prolate_spheroidal_kernel(
        oversampling, support, n_xy_padded
    )
    ...
    weight_avg = xr.DataArray.expand_dims(
        (weight[:, :, 0] + weight[:, :, 1]) / 2, dim=["n_pol"], axis=2
    ...
    vis_grid = np.zeros(
        (n_imag_chan, n_imag_pol, n_xy_padded[0], n_xy_padded[1]), dtype=np.complex128
    )  # Creating an empty grid

    chan_map = (np.zeros(n_chan)).astype(
        np.int
    )  # np.arange(0,n_chan) for cube, n_chan array which maps to the number of channels in image (cube)
    ...
    start = time.time()
    grids_and_sum_weights = da.blockwise(
        serial_grid_dask_sparse,
        ("n_time", "n_switch", "n_imag_chan", "n_imag_pol", "n_u", "n_v"),
        grid_data,
        ("n_time", "n_baseline", "n_chan", "n_pol"),
        uvw,
        ("n_time", "n_baseline", "uvw"),
        weight_avg,
        ("n_time", "n_baseline", "n_pol"),
        flag_row,
        ("n_time", "n_baseline"),
        flag,
        ("n_time", "n_baseline", "n_vis_chan", "n_vis_pol"),
        new_axes={
            "n_switch": 2,
            "n_imag_chan": n_imag_chan,
            "n_imag_pol": n_imag_pol,
            "n_u": n_xy_padded[0],
            "n_v": n_xy_padded[1],
        },
    ...
    uncorrected_dirty_image = fftshift(ifft2(ifftshift(vis_grid[0, 0, :, :]))) * (
        (n_xy_padded[0] * n_xy_padded[1]) / np.sum(sum_weight)
    )
    ...
    corrected_dirty_image = corrected_dirty_image[
    ...
    corrected_dirty_image = corrected_dirty_image / np.max(
        np.abs(corrected_dirty_image)
    )
    casa_corrected_dirty_image = casa_corrected_dirty_image / np.max(
        np.abs(casa_corrected_dirty_image)
    )

    # Calculate max error
    max_error_corrected_dirty_image = np.max(
        np.abs(corrected_dirty_image - casa_corrected_dirty_image)
    )

    # Calculate root mean square error
    rms_error_corrected_dirty_image = np.linalg.norm(
        corrected_dirty_image - casa_corrected_dirty_image, "fro"
    )
    ...
    print(
        "Gridded and image values have been normalized before calculating error values"
    )
    print(
        "Max error between CASA and CNGI dirty images ", max_error_corrected_dirty_image
    )
    print(
        "RMS error between CASA and CNGI dirty images ", rms_error_corrected_dirty_image
    )
    if (max_error_corrected_dirty_image < 1.2886e-07) and (
        rms_error_corrected_dirty_image < 1.616e-06
    ):
        print("Test Pass")
        pass_test = True
    else:
